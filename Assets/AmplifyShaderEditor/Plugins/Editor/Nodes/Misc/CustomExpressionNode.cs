...
{
	[Serializable]
	[NodeAttributes( "Custom Expression", "Misc", "Creates a custom expression or function if <b>return</b> is detected in the written code." )]
	public sealed class CustomExpressionNode : ParentNode
	{
		...
		void DrawInputs()
		{
			...
			for ( int i = 0; i < count; i++ )
			{
				m_foldoutValuesFlags[ i ] = EditorGUILayoutFoldout( m_foldoutValuesFlags[ i ], m_foldoutValuesLabels[ i ] +" - " + m_inputPorts[ i ].Name );
